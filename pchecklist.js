...
var passwordCheckOld = function () 
{
    ...
    stringOutput = ""; //sets stringOutput to blank 
 
 if (stringPassword.length <8) //test that the password is more than 8 characters
 {
    stringOutput = "Password is too short"; //error statement when the password is less than 8 characters
 }
   else
      {
         for (var i = 0; i < arrayBadPasswords.length; i++) //loop comparing the password to the array
         {
            if (stringPassword === arrayBadPasswords[i]) //compares password to the array of bad passwords
               {
                  stringOutput = "Password matches list of bad passwords-try again"; //error message when the password matched an item in the array 
               }
         }
         if (stringOutput == "") //statement when the password is not too short and does not match the array 
         {
            stringOutput = "Great password!"; //output when the password passes the two tests above
         }
      }

 $("output").value = stringOutput; //pushes the string output back to the dom 
};
